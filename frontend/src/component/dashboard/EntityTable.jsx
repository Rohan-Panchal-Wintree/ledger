import { LoaderCircle, PencilLine, Trash2, MoreVertical } from "lucide-react";
...
export default function EntityTable({
  ...
  return (
    <>
      <div ref={menuRef} className="hidden overflow-x-auto lg:block">
                        ...
                        {(canEdit || canDelete) && (
                          <div className="relative inline-flex">
                            <button
                              ...
                              className="btn btn-ghost btn-sm btn-circle text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface"
                            >
                              <MoreVertical size={18} />
                            </button>

                            {isMenuOpen ? (
                              <div className="absolute right-0 top-11 z-20 min-w-44 overflow-hidden rounded-2xl border border-outline-variant/15 bg-surface-container-lowest">
                                {canEdit ? (
                                  <button
                                    type="button"
                                    onClick={() => {
                                ...
                                {canDelete ? (
                                  <button
                                    ...
                                    disabled={
                                      isDeletePending &&
                                      deleteTargetId === row._id
                                    }
                                    className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm font-medium text-error transition-colors hover:bg-surface-container-low disabled:cursor-not-allowed disabled:opacity-70"
                                  >
                                    {isDeletePending &&
                                    deleteTargetId === row._id ? (
                                      <LoaderCircle
      ...
      <div className="grid gap-4 p-4 lg:hidden">
        {rows.map((row) => (
          <article
            key={row._id}
            className="rounded-3xl border border-outline-variant/15 bg-surface-container-lowest p-4"
          >
              ...
              <div className="mt-4 flex gap-3">
                ...
                {canDelete ? (
                  <button
                    ...
                    className="flex-1 rounded-full border border-error/20 px-4 py-3 text-sm font-semibold text-error transition-all hover:bg-error/5 disabled:cursor-not-allowed disabled:opacity-70"
                  >
